...
y_label = tf.placeholder(dtype=tf.float32, shape=[None, y_dim])
y_reshape = tf.placeholder(dtype=tf.float32, shape=[None, 1, 1, y_dim])
y_fill = tf.placeholder(dtype=tf.float32, shape=[None, 28, 28, y_dim])
...
# Discriminator
def discriminator(img_in, y_fill, reuse=None, keep_prob=keep_prob):
	...
	with tf.variable_scope("discriminator", reuse=reuse): 
		...
		x = tf.concat([img_in, y_fill], axis=3)
...
# Generator
def generator(z, y_label, keep_prob=keep_prob, is_training=is_training):
	...
	with tf.variable_scope("generator", reuse=None):
		...
		merged_input = tf.concat(values=[z, y_label], axis=1) # Plus 'label'
...
g = generator(Z, y_label, keep_prob, is_training)
d_real = discriminator(X, y_fill)
d_fake = discriminator(g, y_fill, reuse=True)
...
for i in range(n_epoch):
	...
	for j in range(n_digit):
		y_samples[j, j%10] = 1
	samples = sess.run(g, feed_dict = {Z: sample_Z(n_digit, z_dim), y_label: y_samples, keep_prob: 1.0, is_training:False})
	fig = plot(samples)
	...
	for itr in range(n_batch):
		...
		X_mb, y_label_mb = mnist.train.next_batch(batch_size)
		X_mb = X_mb.reshape(-1 ,28, 28, 1)
		y_reshape = tf.reshape(y_label_mb, [batch_size, 1, 1, 10])
		y_fill_mb = y_reshape * tf.ones([batch_size, 28, 28, 10], dtype=tf.float32)
	
		d_real_ls, d_fake_ls, g_ls, d_ls = sess.run([loss_d_real, loss_d_fake, loss_g, loss_d], feed_dict={X: X_mb, Z: n, y_label: y_label_mb, y_fill: y_fill_mb, keep_prob: keep_prob_train, is_training:True})
		...
		if train_d:
			sess.run(optimizer_d, feed_dict={Z: n, X: X_mb, y_label: y_label_mb, y_fill: y_fill_mb, keep_prob: keep_prob_train, is_training:True})
		if train_g:
			sess.run(optimizer_g, feed_dict={Z: n, y_label: y_label_mb, y_fill: y_fill_mb, keep_prob: keep_prob_train, is_training:True})
