"""
    # Author: Sungbum Park (spark0916@ncsoft.com)
    # Last Modified: Feb. 21, 2019
    # Comment: 
        - Desired MNIST image generation modifying vanilla GAN code
"""

import tensorflow as tf
from tensorflow.examples.tutorials.mnist import input_data
import numpy as np
import matplotlib.pyplot as plt
import matplotlib.gridspec as gridspec
import os

mnist = input_data.read_data_sets('../../MNIST_data', one_hot=True)

img_dim = mnist.train.images.shape[1]
y_dim = mnist.train.labels.shape[1]
z_dim = 100 # Latent space dimension
target_digit = 4


# GPU setting
gpu_config = tf.ConfigProto()
gpu_config.gpu_options.allow_growth=True

# Hyper parameters
batch_size = 128
n_hidden_D = 128
n_hidden_G = 128
n_epoch = 100
n_batch = int(mnist.train.images.shape[0] / batch_size)

X = tf.placeholder(tf.float32, shape=[None, img_dim])
y = tf.placeholder(tf.float32, shape=[None, y_dim])
Z = tf.placeholder(tf.float32, shape=[None, z_dim])

# Initialization
def xavier_init(size):
    in_dim = size[0]
    xavier_stddev = 1. / tf.sqrt(in_dim / 2.)
    return tf.random_normal(shape=size, stddev=xavier_stddev)

D_W1 = tf.Variable(xavier_init([img_dim + y_dim, n_hidden_D]))
D_b1 = tf.Variable(tf.zeros(shape=[n_hidden_D]))
D_W2 = tf.Variable(xavier_init([n_hidden_D, 1]))
D_b2 = tf.Variable(tf.zeros(shape=[1]))
theta_D = [D_W1, D_W2, D_b1, D_b2]

G_W1 = tf.Variable(xavier_init([z_dim + y_dim, n_hidden_G])) # Plus 'label'
G_b1 = tf.Variable(tf.zeros(shape=[n_hidden_G]))
G_W2 = tf.Variable(xavier_init([n_hidden_G, img_dim]))
G_b2 = tf.Variable(tf.zeros(shape=img_dim))
theta_G = [G_W1, G_W2, G_b1, G_b2]


# Discriminator
def discriminator(x, y):
    """ GAN discriminator
    # Arguments
        x: MNIST image
        y: Label
    # Returns
        D_prob: Probability of real MNIST image
    """    
    inputs = tf.concat(values=[x, y], axis=1) # Plus 'label'
    D_h1 = tf.nn.relu(tf.matmul(inputs, D_W1) + D_b1) # (1, 784) x (784, 128) + (1, 128)
    D_logit = tf.matmul(D_h1, D_W2) + D_b2 # (1, 128) x (128, 1) + (1, 1)
    D_prob = tf.nn.sigmoid(D_logit)

    return D_prob, D_logit

# Generator
def generator(z, y):
    """ Conditional GAN generator
    # Arguments
        z: Noise vector for the G(Z)
        y: Label

    # Returns
        G_prob: 786-dimensional vector, which is MNIST image (28x28)
    """
    inputs = tf.concat(values=[z, y], axis=1) # Plus 'label'
    G_h1 = tf.nn.relu(tf.matmul(inputs, G_W1) + G_b1) # (1 x 101) x (101 x 128) + (1 x 128)
    G_log_prob = tf.matmul(G_h1, G_W2) + G_b2 # (1 x 128) x (128 x 784) + (1 x 784)
    G_prob = tf.nn.sigmoid(G_log_prob)

    return G_prob

# Uniform prior generation for G(Z)
def sample_Z(m, n):
    return np.random.uniform(-1., 1., size=[m, n])


G_sample = generator(Z, y) # Generated sample
D_real, D_logit_real = discriminator(X, y) # Discriminator output for real sample
D_fake, D_logit_fake = discriminator(G_sample, y) # Discriminator output for fake sample, which is from the generator


# Loss function
"""
    # Maximization Stage
    arg max_D {log D(x) + [1 - log D(G(z))]}

    # Minimization Stage
    arg min_G {1 - log D(G(z))}
"""
D_loss_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit_real, labels=tf.ones_like(D_logit_real)))
D_loss_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit_fake, labels=tf.zeros_like(D_logit_fake)))
D_loss = D_loss_real + D_loss_fake
G_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit_fake, labels=tf.ones_like(D_logit_fake)))


# Solver
D_solver = tf.train.AdamOptimizer().minimize(D_loss, var_list=theta_D)
G_solver = tf.train.AdamOptimizer().minimize(G_loss, var_list=theta_G)


# Plot
def plot(samples):
    fig = plt.figure(figsize=(4, 4))
    gs = gridspec.GridSpec(4, 4)
    gs.update(wspace=0.05, hspace=0.05)

    for i, sample in enumerate(samples):
        ax = plt.subplot(gs[i])
        plt.axis('off')
        ax.set_xticklabels([])
        ax.set_yticklabels([])
        ax.set_aspect('equal')
        plt.imshow(sample.reshape(28, 28), cmap='Greys_r')

    return fig


history = {'val_D_loss': [],
           'val_G_loss': []}



sess = tf.Session(config=gpu_config)
sess.run(tf.global_variables_initializer())

if not os.path.exists('out/'):
    os.makedirs('out/')

hist = 0

for i in range(n_epoch):
    # MNIST image generation
    y_samples = np.zeros(shape=[16, y_dim])
#    y_samples[:, target_digit] = 1
    for j in range(16):
        y_samples[j, j%10] = 1    
    samples = sess.run(G_sample, feed_dict={Z: sample_Z(16, z_dim), y: y_samples})
    fig = plot(samples)
    plt.savefig('out/{}.png'.format(str(i).zfill(3)), bbox_inches='tight')
    plt.close(fig)


    # Training
    for itr in range(n_batch):
        hist += 1
        X_mb, y_mb = mnist.train.next_batch(batch_size)
        _, D_loss_curr = sess.run([D_solver, D_loss], feed_dict={X: X_mb, Z: sample_Z(batch_size, z_dim), y: y_mb})
        _, G_loss_curr = sess.run([G_solver, G_loss], feed_dict={Z: sample_Z(batch_size, z_dim), y: y_mb})

        history['val_D_loss'].append(D_loss_curr)
        history['val_G_loss'].append(G_loss_curr)   


    # Loss update
    print('Epoch: {}'.format(i))
    print('D Loss: {:.4}'.format(D_loss_curr))
    print('G_loss: {:.4}'.format(G_loss_curr))
    print()

# Graph plot
fig_graph = plt.figure()

sub_D_loss = fig_graph.add_subplot(211)
sub_D_loss.plot(range(hist), history['val_D_loss'], color='red')
sub_D_loss.set_xlabel('Discriminator Loss')



sub_G_loss = fig_graph.add_subplot(212)
sub_G_loss.plot(range(hist), history['val_G_loss'], color='blue')
sub_G_loss.set_xlabel('Generator Loss')

sub_D_loss.grid(linestyle='--', color='lavender')
sub_G_loss.grid(linestyle='--', color='lavender')
 
plt.savefig('out/conditional_gan_loss.png')
plt.close(fig_graph)