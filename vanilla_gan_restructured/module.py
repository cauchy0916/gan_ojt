"""
	...
	# Comment
	   : Conditional GAN for MNIST - module
"""
...
def discriminator(image, label, reuse=None):
	...
	with tf.variable_scope("discriminator", reuse=reuse):
		image = tf.concat(values=[image, label], axis=1)
		h0 = tf.layers.dense(image, units=128, activation=tf.nn.relu)
		out = tf.layers.dense(h0, units=1, activation=None)	

	return out

def generator(latent_vector, label, img_dim):

	with tf.variable_scope("generator", reuse=None):
		z = tf.concat(values=[latent_vector, label], axis=1)
		h0 = tf.layers.dense(z, units=128, activation=tf.nn.relu)
		image = tf.layers.dense(h0, units=img_dim, activation=tf.nn.sigmoid)
